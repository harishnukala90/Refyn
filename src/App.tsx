...
export default function App() {
  ...
  const handleRefine = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err?.message || "Something went wrong while connecting to the prompt refinement engine.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // Copy trigger
  const copyToClipboard = (text: string, type: "refined" | "negative") => {
            ...
            <button
              ...
              className={`p-2 rounded-lg border flex items-center justify-center transition-all cursor-pointer shadow-sm
                ${isDark 
                  ...
                  : "border-sky-200 bg-white text-sky-700 hover:text-sky-950 hover:bg-sky-55"
                }`}
                    ...
                    {sources && sources.length > 0 && (
                      <div className={`px-5 py-3 border-t text-xs transition-colors duration-300
                        ${isDark 
                          ...
                          : "border-sky-100/60 bg-sky-50/20 text-sky-850"}`}>
