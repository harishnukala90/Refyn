...
function getDynamicFallbackPrompt(category: string): string {
  const catKey = category && FALLBACK_PROMPTS[category] ? category : "coding";
...
function localFallbackRefine(rawPrompt: string, category: string, targetModel: string, format: string) {
  ...
  const personas: Record<string, string[]> = {
    ...
    writing: [
      "Master Speachwriter, Storyteller, and Senior Columnist known for high-impact rhythmic sentence variation",
  ...
  const catPersonas = personas[cat] || defaultPersonas;
  ...
  const specsPool = cat === "coding" ? technicalSpecs : cat === "image generation" ? aestheticSpecs : writingSpecs;
  ...
  const suggestionsPool = cat === "coding" ? codeSuggestions : cat === "image generation" ? imageSuggestions : generalSuggestions;
...
- ${lengthGuideline}`;
  ...
  return {
    ...
    improvements: [
      ...
      `Synthesized using Dynamic-Seed procedural compiler (version v2.1.4)`
    ]
  };
}
...
app.post("/api/refine", async (req, res) => {
  const { rawPrompt, category, targetModel, format } = req.body;
...
Raw Prompt: """${rawPrompt}"""\n`;
    ...
    if (category) {
      userInstruction += `Expected Category: ${category}\n`;
    }
    if (targetModel) {
      userInstruction += `Target AI Model: ${targetModel}\n`;
    }
    if (format) {
      userInstruction += `Desired Output Style/Length: ${format}\n`;
    }
...
10. Leverage Google Search grounding via the googleSearch tool to locate the latest standard specifications, code libraries, trending patterns, document definitions, or domain standards relevant to the raw prompt topic. Always optimize prompts dynamically based on up-to-date information.`;
    ...
    if (errorMessage.includes("RESOURCE_EXHAUSTED") || errorMessage.includes("quota") || errorMessage.includes("429")) {
      console.warn("⚠️  [Refyn Server API] Live Gemini API Quota Exceeded (429 Rate Limit). Quietly falling back to high-performance local synthesis engine.");
    } else {
      console.warn("⚠️  [Refyn Server API] Live Gemini API call failed. Falling back to local synthesis engine. Reason:", errorMessage.slice(0, 150));
    }
    try {
      const fallbackResult = localFallbackRefine(rawPrompt, category, targetModel, format);
...
5. Do NOT repeat simple or common tasks (e.g. avoid 'how to build a todo list', 'simple calculator', or basic 'hello world'). Instead, invent fresh, interesting, unique modern topics in the target category (e.g. smart farming, immersive narratives, neural networks, indie game design, automated marketing dashboards, IoT robotics).`;
    ...
    const randomSeedText = `Seed value: ${seed || Math.random()}`;

    const response = await ai.models.generateContent({
      model: "gemini-3.5-flash",
      contents: `Generate a completely unique and specificraw messy prompt for category: "${category || "general"}" with the unique ${randomSeedText}. Ensure it represents a complex or interesting modern technical or creative project.`,
      config: {
        systemInstruction,
    ...
    const errorMessage = error?.message || String(error);
    if (errorMessage.includes("RESOURCE_EXHAUSTED") || errorMessage.includes("quota") || errorMessage.includes("429")) {
      console.warn("⚠️  [Refyn Server API] Live Gemini API Quota Exceeded (429 Rate Limit) for examples. Quietly falling back to curated dynamic catalog.");
    } else {
      console.warn("⚠️  [Refyn Server API] Live Gemini API call for examples failed. Falling back to curated catalog. Reason:", errorMessage.slice(0, 150));
    }
    try {
      const fallbackPrompt = getDynamicFallbackPrompt(category);
